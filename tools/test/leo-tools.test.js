...
describe('leo-tools', () => {
  ...
  it('can find all bots', async () => {
    ...
    assert.isNotEmpty(bots)
  })

  it('can find all queues', async () => {
    const leoTools = new Tools(testConfig)
    const queues = await leoTools.getQueues()
    assert.isNotEmpty(queues)
  })

  // This will make serverless-leo dependent on leo-sdk
  // - in an effort to keep serverless-leo slim we should break out the "tools" into serverless-leo-tools plugin
  // it('can publish events from a file', async () => {
  //   const queueName = 'test-queue'
  //   const leoTools = new Tools(testConfig)
  //   var eventsFilePath = path.resolve('events.json')
  //   await leoTools.publish(queueName, eventsFilePath)
  //   const events = await leoTools.getEvents(queueName)
  // })
  ...
  it('can query a bot with instances', async () => {
    ...
    expect(bots.Items).to.be.an('array').lengthOf(2)
    expect(bots.Items[0]).to.have.any.keys('instances')
    expect(bots.Items[1]).to.have.any.keys('instances')
  }).timeout(5000)
